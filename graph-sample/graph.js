...
module.exports = {
  ...
  getMessages: async function (msalClient, userId, start, end, timeZone) {
    const client = getAuthenticatedClient(msalClient, userId);
    ...
    return (
      client
        ...
        .select(
          "subject,from,replyTo,hasAttachments,bodyPreview,createdDateTime,body"
        )
        // Get at most 50 results
        .top(50)
        .get()
  ...
  getMessagesDetails: async function (msalClient, userId, messageId, timeZone) {
    const client = getAuthenticatedClient(msalClient, userId);
